  ...
  try {
    ...
    const breedingRecord = await prisma.breeding.create({
      data: {
        ...
        numberOfOffspring: data.numberOfOffspring ? parseInt(data.numberOfOffspring) : null,
        cost: data.cost ? parseFloat(data.cost) : null,
        notes: data.notes || null,
        maleAnimals: {
          create: data.maleAnimalIds?.map((id: string) => ({ animalId: id })) || []
        },
        femaleAnimals: {
          create: data.femaleAnimalIds?.map((id: string) => ({ animalId: id })) || []
        }
      }
    })
